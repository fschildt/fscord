...
s32
sys_get_window_events(Sys_Window *sys_window, Sys_Event *events, s32 max_event_count)
{
    Win32Window *win32_window = (Win32Window*)sys_window;
    s32 event_count = g_win32_window_service.GetWindowEvents(win32_window, events, max_event_count);
    return event_count;
}
...
bool
sys_send(Sys_Tcp *sys_tcp, void *buffer, s64 size)
{
    assert(size <= S32_MAX);

    Win32Tcp *win32_tcp = (Win32Tcp*)sys_tcp;
    bool sent = win32_tcp->Send(buffer, (s32)size);
    return sent;
}

s64
sys_recv(Sys_Tcp *sys_tcp, void *buffer, s64 size)
{
    assert(size <= S32_MAX);

    Win32Tcp *win32_tcp = (Win32Tcp*)sys_tcp;
    int recvd = win32_tcp->Recv(buffer, (s32)size);
    return recvd;
}
